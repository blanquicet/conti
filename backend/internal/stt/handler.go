...
// Convert to WAV PCM 16kHz mono with ffmpeg
...
h.logger.Error("ffmpeg failed", "error", err, "stderr", stderr.String(), "user_id", user.ID)
w.WriteHeader(http.StatusBadRequest)
json.NewEncoder(w).Encode(map[string]string{"error": "no se pudo procesar el audio"})
return
}

// Read WAV and validate size
...
text, err := h.transcribe(r.Context(), wavData)
if err != nil {
...
func (h *Handler) transcribe(ctx context.Context, wavData []byte) (string, error) {
...
req, err := http.NewRequestWithContext(reqCtx, "POST", url, bytes.NewReader(wavData))
if err != nil {
